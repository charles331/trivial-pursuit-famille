...
for (const question of QUESTIONS_DATABASE) {
  ...
  if (new Set(question.options.map((option) => normalize(option) || option.trim().toLowerCase())).size !== 4) {
...
for (const categoryId of CATEGORIES) {
  ...
  if (counts[0] < 135 || counts[1] < 135 || counts[2] < 135) {
    errors.push(`${categoryId} doit contenir au moins 135 questions par niveau (enfant/ado/adulte)`);
  }
  if (rows.length < 405) {
    errors.push(`${categoryId} contient ${rows.length} questions au lieu d'au moins 405`);
  }
}
